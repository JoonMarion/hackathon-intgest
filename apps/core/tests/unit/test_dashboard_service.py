...
class DashboardServiceChartTests(BaseIntegrationTestCase):
    ...
    def test_get_chart_data_swapped_dates(self):
        date_range = DateRange(
            date_from=date(2026, 4, 1), date_to=date(2026, 1, 1)
        )
        service = DashboardService(user=self.user, date_range=date_range)
        chart = service.get_chart_data()
        # Dates get swapped internally so chart still produces valid labels
...
class DashboardServiceCategoryBreakdownTests(BaseIntegrationTestCase):
    def setUp(self):
        self.user = UserFactory(
            username="breakdown-user", email="breakdown-user@example.com"
        )
    ...
    def test_get_category_breakdown_expense(self):
        ...
        food_idx = breakdown.labels.index("Food")
        transport_idx = breakdown.labels.index("Transport")
        self.assertEqual(breakdown.data[food_idx], 80.0)
        self.assertEqual(breakdown.data[transport_idx], 40.0)
...
class DashboardServiceRecentTransactionsTests(BaseIntegrationTestCase):
    def setUp(self):
        self.user = UserFactory(
            username="recent-user", email="recent-user@example.com"
        )
        self.cat = CategoryFactory(
            user=self.user, kind=Category.Kind.EXPENSE, name="Misc"
        )
...
class DashboardServiceOrchestratorTests(BaseIntegrationTestCase):
    def setUp(self):
        self.user = UserFactory(
            username="orch-user", email="orch-user@example.com"
        )
